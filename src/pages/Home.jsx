        ...
        <section id="provas" className="max-w-7xl mx-auto px-4 py-24">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                {stats.map((stat, index) => (
                    <div key={index} className="text-center p-6 rounded-2xl bg-neutral-100 border border-zinc-800">
        ...
        <div className="relative">
            ...
            <form onSubmit={handleSubmit} className="relative rounded-3xl border border-zinc-800 bg-neutral-800/80 backdrop-blur-md p-8 shadow-2xl">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    {status.state !== "idle" && (
                        <p
                            className={`text-sm ${status.state === "success" ? "text-green-400" : status.state === "error" ? "text-red-400" : "text-zinc-400"
                                }`}
                        >
                            {status.message}
                        </p>
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="pointer-events-none absolute -inset-px rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                 style={{ boxShadow: '0 0 0 1px rgba(249,115,22,0.25), 0 8px 30px rgba(249,115,22,0.15)' }} />
          </div>
        ))}
      </div>

      {/* CTA opcional no rodapé do FAQ */}
